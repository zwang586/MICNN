...
for i in range(0,3):
    filename = 'data multiLayerVal\\case_'+str(int(1+i))+'_eta_final_3.txt'
    with open(filename) as f:
        x = f.readlines()
    count = 0
    for j in range(0,ny):
        for k in range(0,nx):
            eta_final[i,k,j] = x[count]
            count = count + 1
for i in range(0,3):
...
#    plt.show()
    ...
    plt.savefig('mutliVal_physical_case_'+str(int(1+i))+'_eta_final_3.jpg', dpi=200, bbox_inches = "tight")
    plt.close()
...
for nn in range(0,3):
    ...
    for layer in range(1,4):
        ...
        for i in range(0,1000):
            [eta1,yH,ymax1] = powder3(eta,nx,ny,layer,thickness,ymax0)
        ...
        ax.axes.get_xaxis().set_visible(False)
        ax.axes.get_yaxis().set_visible(False)
        plt.savefig('mutliVal_yNet_case_'+str(int(nn+1))+'_eta_0_'+str(layer)+'.jpg', dpi=200, bbox_inches = "tight")
#        np.save('multiVal_pre_yNet_'+str(int(nn+1))+'_eta_0_'+str(layer)+'.npy',eta[:,0:305])          
        ...
        ax.axes.get_xaxis().set_visible(False)
        ax.axes.get_yaxis().set_visible(False)
        plt.savefig('mutliVal_yNet_case_'+str(int(nn+1))+'_eta_final_'+str(layer)+'.jpg', dpi=200, bbox_inches = "tight")
#        np.save('mutliVal_yNet_case_'+str(int(nn+1))+'_eta_final_'+str(layer)+'.npy',eta[:,0:305])      
