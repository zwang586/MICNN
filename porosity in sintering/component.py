# -*- coding: utf-8 -*-
####Use yNet to perform component-scale simulation#########################################
####NOTE: Require high-performance computer to generate large-scale powder bed#############
...
#Load slicedCAD data
MNum = np.loadtxt('data slicedCAD\\MLogo_num.txt')
MXstart = np.loadtxt('data slicedCAD\\MLogo_xStart.txt')
MXend = np.loadtxt('data slicedCAD\\MLogo_xEnd.txt')
...
for j in range(1,631):
    ...
    ###############Generate a series of mini-powder beds##########################################
    layerNo = j
...
#            print(k)
            ...
            for i in range(0,1000):
                [eta1Seg,yH,ymax1Seg] = powder3v2_5a(etaSeg,layerSeg,layerH,1,layerH,ymax0Seg,rad_std) #powder3v2(s_eta,nx,ny,layer,t,ymax,rad_std): 
        #        print(yH)
        ...
        if j % 2 == 1: #3, 5, 7 ... layer
            for k in range(1,layerSegNo+1):
                ...
                for i in range(0,1000):
                    [eta1Seg,yH,ymax1Seg] = powder3v2_5a(etaSeg,layerSeg,layerH+30,1,layerH+30,ymax0Seg,rad_std) #powder3v2(s_eta,nx,ny,layer,t,ymax,rad_std): 
...
#                print(k)
                if k == 1:                    
                    ...
                    for i in range(0,1000):
                        [eta1Seg,yH,ymax1Seg] = powder3v2_5a(etaSeg,50,layerH+30,1,layerH+30,ymax0Seg,rad_std) #powder3v2(s_eta,nx,ny,layer,t,ymax,rad_std): 
                        if yH >layerH*1+30: #break when the to-be-added particle higher than layerH*layerNo 
                            break
                        etaSeg = eta1Seg
                        ymax0Seg = ymax1Seg
...
###########################yNet based sintering simulation###########################
    ...
    if j % 2 == 0:
        for iii in range(1,int(MNum[int(j/2)-1])+1):
            if j*layerH < 128: #first one or two layers, where total height less than 128
                ...
                x_test = np.reshape(x_test, (1, nx3, ny3, 1))  # adapt this if using `channels_first` image data format 
        ...
        nn = nn + 1
    testStr = []
    np.savetxt('layer_'+str(layerNo)+'.layer',testStr)        
...
#    plt.savefig('fig5_layer'+str(j)+'_1.jpg', dpi=300, bbox_inches = "tight")   
    if j == 1 or j%50 == 0: ##Save eta data every 50 layers
        np.save('fig5a_eta_layer_'+str(int(j/2))+'.npy',eta)
np.save('fig5a_eta_layer_final.npy',eta)
